...
export default function ChatSection({ setActiveSection }: ChatSectionProps) {
  ...
  useEffect(() => {
    ...
    "What projects has Malvin worked on?",
    "What technologies does Malvin master?",
    "How can I contact Malvin?",
    "Tell me about Malvin's educational background",
    "What is the Goyang Lidah Jogja project?",
    "Can Malvin help with my project?"
  ]
      ...
      >
        <div className="h-[500px] w-full max-w-3xl mx-auto border border-[rgba(var(--foreground),0.1)] rounded-lg shadow-md flex flex-col bg-white dark:bg-black">
          {/* Chat Messages Area */}          <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
            {messages.length === 0 && (              <div className="text-center h-full flex flex-col justify-center">
                <Bot size={32} className="mx-auto mb-4 opacity-50" />
                <p className="mb-2">Hi! I'm here to help you get to know Malvin.</p>
                <p className="text-xs opacity-70 mb-4">Ask about his projects, skills, or experience!</p>
          ...
          <div className="border-t border-[rgba(var(--foreground),0.1)] p-4">
            <form onSubmit={handleSubmit} className="flex gap-2">              <input
                value={input}
                onChange={handleInputChange}
                placeholder="Ask me anything about Malvin..."
